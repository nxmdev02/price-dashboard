...
<template>
  ...
  <section class="card products-card compare-card"><div class="card-head product-head"><div><h3>전체 경쟁사 가격 비교</h3><p>배송비와 옵션 추가금을 포함한 최종 가격</p></div><div class="price-sort-links"><button :class="{ active: sort === 'low' }" @click="sort = 'low'">낮은 가격순</button><span>·</span><button :class="{ active: sort === 'high' }" @click="sort = 'high'">높은 가격순</button></div></div>
    ...
    <div class="table-wrap"><table><thead><tr><th>모델명</th><th>경쟁사</th><th class="right">최종 비교가격</th><th>상태</th><th>상품 링크</th></tr></thead><tbody><tr v-for="mapping in paginatedMappings" :key="mapping.productId + mapping.id"><td class="model-name-cell"><strong>{{ mapping.modelCode }}</strong><span class="model-details"> · W{{ mapping.modelWidth }} × D{{ mapping.modelDepth }} × H{{ mapping.modelHeight }} · 드롭다운 도어 {{ mapping.modelDoorCount }}개</span></td><td>{{ mapping.companyName }}</td><td class="right price">{{ money(mapping.latestPrice) }}</td><td><span class="status-pill" :class="statusClass(mapping.lastCollectionStatus)">{{ statusText(mapping.lastCollectionStatus) }}</span></td><td><a v-if="mapping.productUrl" :href="mapping.productUrl" target="_blank" rel="noopener" aria-label="상품 링크" title="상품 링크"><ExternalLink :size="17"/></a></td></tr><tr v-if="!filteredMappings.length"><td colspan="5" class="empty">조건에 맞는 가격 정보가 없습니다.</td></tr></tbody></table></div>
